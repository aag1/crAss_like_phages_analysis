...
hmm_file=MCP_portal_TerL.hmm
if [ -f ${hmm_file} ]; then rm -rf ${hmm_file}; fi


for prot in 'MCP' 'portal' 'TerL'
do
    ...
    do
        ...
        if [ ${name} == 'MCP_MCP' ]
        then
            hmmbuild -n 'MCP_MCP' 'MCP_MCP.hmm' /data/umcg-tifn/DATABASES/data_Yutin_2017/crassphage_2017/prot_align/MCP.afa

            cat 'MCP_MCP.hmm' >> ${hmm_file}
